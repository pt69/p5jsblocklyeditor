...
Blockly.JavaScript['adt_queue_head'] = function(block) {
  var value_varName = Blockly.JavaScript.valueToCode(block, 'queueVariable', Blockly.JavaScript.ORDER_ATOMIC);  
  var code = '$~' + value_varName + '.head()' + '§#erstes Element von ' + value_varName + ' (lösche es nicht!) #§~$';
  return [code, Blockly.JavaScript.ORDER_ATOMIC];
};
...
Blockly.JavaScript['adt_queue_enqueue'] = function(block) {
  ...
  var code = list + '.enqueue(' + extrahiereCode(neuesElement) + ');\n' + '§#An ' + list + ' wird ' + extrahiereStrukt(neuesElement) + ' angehängt.#§';
  return code;
};
...
Blockly.JavaScript['adt_queue_dequeue'] = function(block) {
  var value_varName = Blockly.JavaScript.valueToCode(block, 'queueVariable', Blockly.JavaScript.ORDER_ATOMIC);  
  var code = '$~' + value_varName + '.dequeue()' + '§#erstes Element von ' + value_varName + ' (lösche es!)#§~$';
  return [code, Blockly.JavaScript.ORDER_ATOMIC];
};
...
Blockly.JavaScript['adt_queue_getQueue'] = function(block) {
  var value_varName = Blockly.JavaScript.valueToCode(block, 'queueVariable', Blockly.JavaScript.ORDER_ATOMIC);  
  var code = '$~' + value_varName + '.getQueue()' + '§#gesamter Inhalt von ' + value_varName + ' als Array#§~$';
  return [code, Blockly.JavaScript.ORDER_ATOMIC];
};